...
gulp.task('create', function(){
  inquirer.prompt([{name: 'appName', message: '请输入webApp的名称:', type: 'input'}, 
                   {name: 'appHost', message: '请输入app的域名:', type: 'input'},  
                   {name: 'appPort', message: '请输入端口:', type: 'input'},
                   {name: 'appFePath',  message: '请输入webApp的前端路径:', type: 'input'}], 
  function( answers ) {
    ...
    var setWebs = function(exist){
      ...
      console.log('恭喜，已创建完毕，可运行 gulp ' + answers.appName + '运行！');
    }

    //如果有同名则提示
    fs.readdir(websSrcPath, function(err, files){
      console.log(!!~files.indexOf(answers.appName), 'test');
